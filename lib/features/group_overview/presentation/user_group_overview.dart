...
class UserGroupOverview extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final isInUserGroup = ref.watch(userGroupServiceProvider.select((e) => e.value?.any((t) => t.groupId == groupAsync.valueOrNull?.groupId)));
    return groupAsync.when(
      data: (data) {
        if (data == null) {
          return const Center(child: CircularProgressIndicator());
        } else if (isInUserGroup != null && isInUserGroup) {
          return GroupOverview(groupId: groupId,actions: [PopUpMenuLeave(groupDto: data)]);
