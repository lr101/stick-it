...
class UserProfile extends ConsumerWidget {
  ...
  Widget _buildQuickStats(AsyncValue<dynamic> userPins, WidgetRef ref) {
    return Row(
      ...
      children: [
        _statItem("Sticks", userPins.whenOrNull(data: (d) => d.length.toString()) ?? "---"),
        _statItem("Groups", ref.watch(userGroupServiceProvider).value?.length.toString() ?? "---"),
      ],
    );
  }
  ...
  List<SliverToBoxAdapter> _buildDetailList(AsyncValue<UserEntity?> currentUser, AsyncValue<UserLikesDto> likes) {
    return [
